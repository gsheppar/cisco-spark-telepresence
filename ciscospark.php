...
file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_rec_message.txt', $pythonreturn.PHP_EOL, FILE_APPEND);

if ((strpos($pythonreturn, ':help') !== false) and ($personId != $sparkbot)) {
	$response = file_get_contents('$directory/Spark/TXT/help.txt');
	file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_send_message.txt', $response);
...
if ((strpos($pythonreturn, ':message') !== false) and ($personId != $sparkbot)) {
	...
	$xml = simplexml_load_file(dirname(__FILE__).'/Spark/XML/codec_systems_list.xml');	
	...
	$codec_message = simplexml_load_file(dirname(__FILE__).'/Spark/XML/codec_message.xml');
	$codec_message->UserInterface[0]->Message->Prompt->Display->Text = $message_reply;
	file_put_contents(dirname(__FILE__).'/Spark/XML/codec_message.xml', $codec_message->asXML());
...
if ((strpos($pythonreturn, ':question') !== false) and ($personId != $sparkbot)) {
	...
	$message_reply = str_replace($codec_name, '', $message_reply);
	$message_reply = substr($message_reply, 1);
	$codec_question = simplexml_load_file(dirname(__FILE__).'/Spark/XML/codec_question.xml');
	$codec_question->UserInterface[0]->Message->Prompt->Display->Text = $message_reply;
	file_put_contents(dirname(__FILE__).'/Spark/XML/codec_question.xml', $codec_question->asXML());
...
if ((strpos($pythonreturn, ':dial') !== false) and ($personId != $sparkbot)) {
	...
	$xml_change = simplexml_load_file(dirname(__FILE__).'/Spark/XML/codec_dial.xml');
	$xml_change->Dial->Number = $codec_command;
	file_put_contents(dirname(__FILE__).'/Spark/XML/codec_dial.xml', $xml_change->asXML());
...
if ((strpos($pythonreturn, ':diag') !== false) and ($personId != $sparkbot)) {
	...
	$xml_diag = simplexml_load_file(dirname(__FILE__).'/Spark/XML/status_diag.xml');
	$codec_name = $codec_name. " Diagnostics Report";
	file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_send_message.txt', $codec_name.PHP_EOL);
	foreach ($xml_diag->Diagnostics->Message as $diag) {	
		$response = "-- " .$diag->Description;
		file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_send_message.txt', $response.PHP_EOL, FILE_APPEND);
	}
	exec ("python $directory/Spark/sparkmessfromfile.py");
}
...
if ((strpos($pythonreturn, ':list') !== false) and ($personId != $sparkbot)) {
	...
	file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_send_message.txt', $response.PHP_EOL);
	foreach ($xml as $devicelist):
        $devicename= "-- " .$devicelist['id'];
        file_put_contents(dirname(__FILE__).'/Spark/TXT/spark_send_message.txt', $devicename.PHP_EOL, FILE_APPEND);	
    endforeach;
    exec ("python $directory/Spark/sparkmessfromfile.py");
}
